...
public class DatabaseSupport {
    ...
    Context context;
    public DatabaseSupport(MainActivity context) {
    ...
    private void processSQLite() {
        ...
        try{
            ...
            int length;
            while ((length = databaseInputStream.read(buffer)) > 0){ databaseOutputStream.write(buffer,0,length);
            }
    ...
    public void insertStudent(Student student){
        ...
        if(db.insert("Student",null,contentValues) > 0){ Toast.makeText(context, "successful", Toast.LENGTH_SHORT).show();
        }
    }
